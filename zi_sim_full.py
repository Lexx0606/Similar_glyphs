...
if lang_code == "ru":
    ...
    hsk_check_title = "Показывать только HSK 3.0"
else:
    ...
    hsk_check_title = "Show HSK 3.0 only"
...
try:
    ...
    for row in rows:
        db.execute(
           "INSERT INTO vec_items(zi, pinyin, ru_short_art, ru_full_art, eng_art, hsk, timm_embedding, res_embedding) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
hsk_select = st.sidebar.checkbox (hsk_check_title, value = True)
...
if embeding_select == timm_name:
    ...
    if result:
        # Если найдена строка, выводите соответствующее значение из столбца embedding
        query = result[0]
        if hsk_select:
            rows = db.execute("""SELECT zi, distance, pinyin, ru_short_art, ru_full_art, eng_art FROM vec_items WHERE hsk=True AND timm_embedding MATCH ? ORDER BY distance LIMIT ?""", [deserialize_f32(query), zi_count],).fetchall()
        else:
        ...
        # Если строка не найдена, выводите сообщение об этом
        st.error(find_eror)
else:
    ...
    if result:
        ...
        query = result[0]
        if hsk_select:
            rows = db.execute("""SELECT zi, distance, pinyin, ru_short_art, ru_full_art, eng_art FROM vec_items WHERE hsk=True AND res_embedding MATCH ? ORDER BY distance LIMIT ? """, [deserialize_f32(query), zi_count],).fetchall()
        else:
